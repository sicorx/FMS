...
extern unsigned char zbus_check_sum(unsigned char *buff, int len)
{
    ...
    for(i=3; i<len; i+=2)
    {
        crc += ((get_hex(buff[i]) << 4) | get_hex(buff[i+1]));
    }

    return crc;
}

static unsigned short itts2_crc_table[256] = {
 ...
 * Parameters:
 * Returns:
 *      이노튜브 온습도 IT-TS2 CRC16
 */
unsigned short itts2_crc16(unsigned char *ptr,unsigned short size)
{
    ...
    for(j=0; j<size; j++)
    {
        i = ((unsigned short)(crc_accum >> 8) ^ *ptr++) & 0xff;
        crc_accum = (crc_accum << 8) ^ itts2_crc_table[i];
    }
